...
public/og.png         social card, generated by make og from scripts/make-og.swift
```

The social card is not a mockup — `scripts/make-og.swift` runs the same synthetic signal through the
same magma colormap and renders a real spectrogram with Core Graphics. Regenerate it with `make og`
